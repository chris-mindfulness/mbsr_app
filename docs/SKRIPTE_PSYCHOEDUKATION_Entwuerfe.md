# Psychoedukation: Alle Texte (readingCards + Audio-Clips + Snippets)
...
# Teil 1: readingCards (Abschnitt PSYCHOEDUKATION in der App)
...
# Teil 2: Audio-Clips (infoClips.psychoedukation — noch offen)

Arbeitsvorlagen für ElevenLabs-Clips (ca. 60–90 s).

## 1. Woche 1 — „Laktat des Geistes"

*Kanal: `infoClips.psychoedukation` Woche 1 | Exzerpt-Ref: 3.1, 2.2*
...
[thoughtful] Du musst nichts damit tun. Es reicht, das Unbehagen wahrzunehmen und bei der Übung zu bleiben, so gut es gerade geht. Die beste Praxis ist die, die du tatsächlich machst – auch wenn sie kurz ist.

## 2. Woche 4 — Physiologischer Seufzer

*Kanal: `infoClips.psychoedukation` Woche 4 | Exzerpt-Ref: Hub-Solo, Balban et al. 2023*
...
## 3. Woche 2 — States → Traits

*Kanal: `infoClips.psychoedukation` Woche 2 | Exzerpt-Ref: 2.1 | Prio B*

[thoughtful] Unser Erleben wechselt ständig: müde, wach, ruhig, gereizt. Zustände kommen und gehen wie Wetter.

[thoughtful] Was sich durch regelmäßige Praxis verändern kann, ist nicht der einzelne Moment, sondern die Ausgangslage. Der Zustand nach dem Üben wird schrittweise zum neuen Normal – zum Trait statt zum State.

[thoughtful] In Davidsons Forschung zeigt sich das als messbare Verschiebung: Praktizierende kehren nach emotionaler Aktivierung schneller zur Baseline zurück. Das Gehirn lernt nicht, weniger zu fühlen, sondern sich schneller zu erholen.
...
# Teil 3: Snippets für Skript-Anreicherung (kein eigener App-Clip)

### 5.1 Physiologischer Seufzer in Sitzmeditation (Woche 4–5)
...
*Einbau in OM-Passage. Exzerpt-Ref: 2.7. Prio A (SA).*

> Wenn du merkst, dass du planst, grübelst oder etwas reparieren willst – du musst das nicht stoppen. Es reicht, zu bemerken: „Ah, der Geist ist gerade beim Planen." Und dann wieder zur offenen Haltung zurück.

### 5.3 Decentering-Hinweis (Woche 5)

*Einbau in Reflexionspassage. Exzerpt-Ref: 1.15 (Creswell 2017). Prio B (SA).*

> Wenn du Gedanken oder Gefühle beobachtest, ohne sofort darauf zu reagieren – genau das nennt die Forschung „Decentering". Du bist nicht weniger beteiligt, aber du hast etwas Abstand gewonnen. Wie jemand, der einen Film sieht und weiß, dass es ein Film ist.

---

# Nächste Schritte

1. **Texte redigieren:** Änderungen in dieser Datei vornehmen, dann nach `lib/app_daten.dart` übertragen.
2. **Audio-Clips produzieren:** Skripte (Teil 2) mit Stilrichtlinien abgleichen, über ElevenLabs rendern, in Appwrite hochladen, `appwrite_id` eintragen.
3. **SA-Snippets einbauen:** Snippets (Teil 3) in die Meditationsskripte unter `avatar/audios_anleitungen/` integrieren.
4. Pro Woche gilt aktuell ein Psychoedukations-Clip-Slot ohne Code-Änderung. Bei mehreren Clips pro Woche: Code-Erweiterung nötig.
